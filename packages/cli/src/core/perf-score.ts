/**
 * 服务端跑分算法 — 与 packages/sdk/src/core/perf-score.ts 保持一致
 * SDK 只负责采集原始数据，服务端负责计算评分
 */
import type {
  PerfRunScore,
  PerfScoreItem,
...
export function scorePerfRun(snapshot: PerformancePayload): PerfRunScore {
  ...
  const items: PerfScoreItem[] = [
    { name: 'FPS', score: Math.round(fpsData.score), weight: 0.2, value: fpsData.value >= 0 ? fpsData.value : null, unit: 'fps', rating: rating(fpsData.score) },
    { name: 'LCP', score: Math.round(lcpScore), weight: 0.15, value: vitalsMap.get('LCP') ?? null, unit: 'ms', rating: rating(lcpScore) },
    { name: 'CLS', score: Math.round(clsScore), weight: 0.1, value: vitalsMap.get('CLS') ?? null, unit: '', rating: rating(clsScore) },
    { name: 'FCP', score: Math.round(fcpScore), weight: 0.1, value: vitalsMap.get('FCP') ?? null, unit: 'ms', rating: rating(fcpScore) },
    { name: 'TTFB', score: Math.round(ttfbScore), weight: 0.1, value: vitalsMap.get('TTFB') ?? null, unit: 'ms', rating: rating(ttfbScore) },
    { name: 'INP', score: Math.round(inpScore), weight: 0.1, value: vitalsMap.get('INP') ?? null, unit: 'ms', rating: rating(inpScore) },
    { name: 'LongTasks', score: Math.round(ltScore), weight: 0.15, value: snapshot.longTasks.length, unit: '次', rating: rating(ltScore) },
    {
      ...
      value: snapshot.resources.length
        ? Math.round(snapshot.resources.reduce((s, r) => s + r.duration, 0) / snapshot.resources.length)
        : null,
      unit: 'ms',
