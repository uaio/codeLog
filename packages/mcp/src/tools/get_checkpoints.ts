...
export const getCheckpoints = {
  ...
  description: `读取设备上由 AI 埋入的 @codelog[checkpoint] 日志，验证关键开发节点是否已执行。

使用场景：
- AI 在代码中埋入 console.log('@codelog[checkpoint] 节点: 描述') 后，让用户跑一遍流程
- 调用此工具读取所有 checkpoint 日志，判断哪些节点被执行了、哪些没有
- 用于验证功能逻辑是否按预期跑通

checkpoint 日志格式约定：
  console.log('@codelog[checkpoint] 节点名称: 描述信息')
  console.log('@codelog[checkpoint] 节点名称: 描述信息', { 附加数据 })

示例：
  console.log('@codelog[checkpoint] login: 用户点击登录按钮')
  console.log('@codelog[checkpoint] login: 接口请求发出', { url: '/api/login' })
  console.log('@codelog[checkpoint] login: token 写入成功', { hasToken: !!token })`,
  ...
  async execute(args: { deviceId?: string; feature?: string; limit?: number }): Promise<unknown> {
    ...
    const checkpoints = logs.filter((log: any) => {
      const text = Array.isArray(log.args)
        ...
        : String(log.message ?? '');
      return text.includes('@codelog[checkpoint]');
    });
    ...
    const result = filtered.map((log: any) => {
      ...
      // 解析 @codelog[checkpoint] 节点名称: 描述
      const match = text.match(/@codelog\[checkpoint\]\s*([^:]+):\s*(.*)/i);
      return {
    ...
    return {
      ...
      summary:
        result.length === 0
          ? '⚠️ 未发现任何 @codelog[checkpoint] 日志，请确认代码中已埋点且用户已执行相关操作'
