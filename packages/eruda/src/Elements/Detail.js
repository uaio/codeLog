...
export default class Detail {
  ...
  _render() {
    ...
    let attributes = `<tr><td>${i18n.t('common.empty')}</td></tr>`
    ...
    attributes = `<h2>${i18n.t('elements.attributes')}</h2>
    ...
    if (!isEmpty(data.styles)) {
      ...
      styles = `<h2>${i18n.t('elements.styles')}</h2>
  ...
  _bindEvent() {
    ...
    this._$container
      ...
      .on('click', c('.refresh'), () => {
        this._render()
        devtools.notify(i18n.t('common.refreshed'), { icon: 'success' })
      })
