...
export default class Console extends Tool {
  ...
  _appendTpl() {
    ...
    $el.append(
      ...
      <div class="control">
        ...
        <span class="level active" data-level="all">${i18n.t('common.all')}</span>
        <span class="level" data-level="info">${i18n.t('console.info')}</span>
        <span class="level" data-level="warning">${i18n.t('console.warning')}</span>
        <span class="level" data-level="error">${i18n.t('console.error')}</span>
      ...
      <div class="js-input">
        <div class="buttons">
          <div class="button cancel">${i18n.t('common.cancel')}</div>
          <div class="button execute">${i18n.t('common.execute')}</div>
        </div>
  ...
  _bindEvent() {
    ...
    $control
      ...
      .on('click', c('.filter'), () => {
        LunaModal.prompt(i18n.t('common.filter')).then((filter) => {
      ...
      .on('click', c('.copy'), () => {
        ...
        container.notify(i18n.t('common.copied'), { icon: 'success' })
      })
  ...
  _initCfg() {
    ...
    settings
      .text(i18n.t('tools.console'))
      .switch(cfg, 'asyncRender', i18n.t('console.asyncRender'))
      .switch(cfg, 'jsExecution', i18n.t('console.jsExecution'))
      .switch(cfg, 'catchGlobalErr', i18n.t('console.catchGlobalErr'))
      ...
      .select(cfg, 'maxLogNum', i18n.t('console.maxLogNum'), [
        i18n.t('console.infinite'),
        '250',
        '125',
        '100',
        '50',
