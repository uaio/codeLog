...
export default class Detail extends Emitter {
  ...
  show(data) {
    ...
    let reqHeaders = `<tr><td>${i18n.t('common.empty')}</td></tr>`
    if (data.reqHeaders) {
    ...
    let resHeaders = `<tr><td>${i18n.t('common.empty')}</td></tr>`
    if (data.resHeaders) {
    ...
    <div class="${c('http')}">
      ${postData}
      <div class="${c('section')}">
        <h2>${i18n.t('network.responseHeaders')}</h2>
      ...
      <div class="${c('section')}">
        <h2>${i18n.t('network.requestHeaders')}</h2>
        <table class="${c('headers')}">
          <tbody>
            ${reqHeaders}
          </tbody>
  ...
  _copyRes = () => {
    ...
    copy(data)
    this._devtools.notify(i18n.t('common.copied'), { icon: 'success' })
  }
  _bindEvent() {
