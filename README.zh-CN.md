...
**codeLog 解决这些问题。** 在 H5 页面嵌入一行代码，手机上发生的一切——日志、请求、存储、性能——实时同步到你的电脑，也同步给 AI Agent，让 AI 能在真实设备上验证它写的每一行代码。

codeLog 有三种用法，按需选择，互相独立：

| 模式 | 适合场景 | 如何使用 |
|------|---------|---------|
| **仅 SDK** | 本地调试，不需要远程 | 引入 SDK，本机直接打开 Eruda 面板 |
| **SDK + PC 面板** | 远程监控，团队协作 | `npx @codelog/cli` 启动服务 |
| **SDK + Claude Code** | AI 辅助开发，自动验证 | 再执行 `npx @codelog/cli init` 配置 MCP |
...
**同时，codeLog 是一个开放数据平台**：任何系统（CI/CD、服务端、Native App、第三方工具）均可通过标准接口推送数据，供 PC 面板展示和 AI 工具查询。
...
| Slash Command | 功能 |
|---|---|
| `/codelog:setup` | **一键从零到就绪**（检测 SDK→注入→启动→确认连接） |
| `/codelog:start` | 启动监控服务 + 建立 WS 连接 |
| `/codelog:stop` | 停止监控服务 |
| `/codelog:status` | 查看设备连接状态 |
| `/codelog:logs` | 查看日志 + checkpoint 链路 |
...
这是 codeLog 的核心场景：AI 工具在开发 H5 功能时，通过真实设备数据验证每个关键节点。
...
Claude 调用 `start_codelog` → 服务启动 → WS 长连接建立 → 自动打开 PC 面板。
...
AI 在代码关键节点自动埋入 `@codelog[checkpoint]` 日志：

```javascript
async function handleLogin(username, password) {
  console.log('@codelog[checkpoint] login: 开始登录', { username })
  try {
    console.log('@codelog[checkpoint] login: 发起请求')
    const res = await api.login(username, password)
    console.log('@codelog[checkpoint] login: 请求成功', { status: res.status })
    localStorage.setItem('token', res.data.token)
    console.log('@codelog[checkpoint] login: token 已保存', { hasToken: true })
    router.push('/home')
    console.log('@codelog[checkpoint] login: 跳转首页')
  } catch (e) {
    console.log('@codelog[checkpoint] login: 请求失败', { error: e.message })
  }
}
```
...
**埋点格式约定：**
```
console.log('@codelog[checkpoint] 功能名: 步骤描述', { 可选附加数据 })
                ^^^^^^^^ codeLog 统一标识符
```

`@codelog` 是 codeLog 所有开发期日志的标识前缀，方便统一查询和清除。
...
**第 5 步：清除 @codelog 日志（验证通过后必须执行）**

```
/codelog:clean
```

Claude 搜索代码中所有 `@codelog` 前缀的 console.log 行并删除。

**`@codelog` 日志是开发期工具，不应进入生产代码。**
...
```bash
npx @codelog/cli          # 启动，自动打印局域网 IP 和 SDK 接入代码
npx @codelog/cli -p 8080  # 指定端口（手机和电脑不在同一 WiFi 时按需调整）
```
...
```bash
npx @codelog/cli init     # 自动检测并配置 Claude Code / Cursor / Windsurf
```

重启 AI 工具后，Claude Code 里直接输入 `/codelog:start` 开始 AI 辅助开发。
...
任何系统均可通过 REST 接口向 codeLog 推送数据，数据将实时出现在 PC 面板和 MCP 工具中。
...
- [x] `@codelog/types` 统一数据标准（Envelope v1）
- [x] `POST /api/ingest` 外部数据接入接口
- [x] `npx @codelog/cli init` 一键配置 AI 工具 + Claude Code slash commands
- [x] `@codelog[checkpoint]` 开发期埋点约定 + `get_checkpoints` 验证工具
- [x] `/codelog:clean` 验证完成后自动清除调试日志
- [x] `start_codelog` / `stop_codelog` 显式生命周期管理
